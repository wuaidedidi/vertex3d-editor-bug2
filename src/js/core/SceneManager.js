...
const SceneManager = (() => {
  ...
  function _initGrid() {
    ...
    axisHelper = _createAxisHelper(5);
    scene.add(axisHelper);
  }

  function _createAxisHelper(size) {
    const group = new THREE.Group();
    const axisRadius = 0.03;

    const xMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
    const xGeometry = new THREE.CylinderGeometry(axisRadius, axisRadius, size, 8);
    ...
    const yGeometry = new THREE.CylinderGeometry(axisRadius, axisRadius, size, 8);
    yGeometry.translate(0, size / 2, 0);
    const yAxis = new THREE.Mesh(yGeometry, yMaterial);
    group.add(yAxis);

    const zMaterial = new THREE.MeshBasicMaterial({ color: 0x0000ff });
    const zGeometry = new THREE.CylinderGeometry(axisRadius, axisRadius, size, 8);
